...
impl Session {
    ...
    /// until a fetch reloads the resources (the first [`Session::fetch_textures`]
    /// that returns a real texture repopulates the whole map; a fetch of only
    /// absent refs does not). Read descriptors right after [`Session::open`], or
    /// after a fetch, not after bare playback.
    ///
